			...
			if conn.requireSigning && !isEncrypted {
				if conn.session != nil {
					if conn.session.sessionFlags&(SMB2_SESSION_FLAG_IS_GUEST|SMB2_SESSION_FLAG_IS_NULL) == 0 {
						// SMB2 interim responses for asynchronously processed
						// requests are explicitly allowed to be unsigned. The
						// request remains outstanding and tryHandle records the
						// AsyncId before the signed final response arrives.
						asyncInterim := p.Flags()&SMB2_FLAGS_ASYNC_COMMAND != 0 && NtStatus(p.Status()) == STATUS_PENDING
						if conn.session.sessionId == p.SessionId() && !asyncInterim {
...
func (conn *conn) tryHandle(pkt []byte, e error) error {
	...
	case NtStatus(p.Status()) == STATUS_PENDING:
		if p.Flags()&SMB2_FLAGS_ASYNC_COMMAND == 0 || p.AsyncId() == 0 {
			err := &InvalidResponseError{"invalid asynchronous pending response"}
			rr.err = err
			close(rr.recv)
			return err
		}
	...
	default:
		if rr.asyncId != 0 {
			if p.Flags()&SMB2_FLAGS_ASYNC_COMMAND == 0 || p.AsyncId() != rr.asyncId {
				err := &InvalidResponseError{"mismatched asynchronous response"}
				rr.err = err
				close(rr.recv)
				return err
			}
		} else if p.Flags()&SMB2_FLAGS_ASYNC_COMMAND != 0 {
			err := &InvalidResponseError{"unexpected asynchronous response"}
			rr.err = err
			close(rr.recv)
			return err
		}
